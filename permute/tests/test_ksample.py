...
def test_worms_ksample_keep_dist():
    worms = data.worms()
    res = k_sample(worms.x, worms.y, stat='one-way anova', reps=1000, seed=1234, keep_dist= True)
    np.testing.assert_array_less(0.006, res[0])
    np.testing.assert_array_less(res[0], 0.02)
    assert len(res) == 3

def test_worms_ksample_callable():
    worms = data.worms()
    res = k_sample(worms.x, worms.y, stat = one_way_anova, reps=1000, seed=1234)
    np.testing.assert_array_less(0.006, res[0])
    np.testing.assert_array_less(res[0], 0.02)
...
def test_testosterone_ksample():
    ...
    res_keep = bivariate_k_sample(x, group1, group2, reps=5000, seed=5, keep_dist = True)
    assert len(res_keep) == 3
    np.testing.assert_array_less(res[0], 0.0002)
    np.testing.assert_array_less(res_keep[0], 0.0002)


def test_testosterone_ksample_stat():
    testosterone = data.testosterone()
    x = np.hstack(testosterone.tolist())
    group1 = np.hstack([[i]*5 for i in range(len(testosterone))])
    group2 = np.array(list(range(5))*len(testosterone))
    ...
    assert len(group2) == 55
    assert len(x) == 55
    res = bivariate_k_sample(x, group1, group2, reps=5000, seed=5, stat = two_way_anova)
    np.testing.assert_array_less(res[0], 0.0002)

test_worms_ksample_keep_dist()
test_testosterone_ksample()
